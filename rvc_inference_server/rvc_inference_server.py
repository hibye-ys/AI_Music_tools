...
def rvc_inference_model(request: rvcInferenceRequest):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for item in s3.list_objects_v2(
            Bucket="s3musicproject", Prefix=f"{request.user_id}/TrainingFiles"
        )["Contents"][1:]:
            file_name = os.path.basename(item["Key"])
            s3.download_file(
                "s3musicproject", item["Key"], os.path.join(logs_path, file_name)
            )
        ...
        output_path = os.path.join(
            file_path, f"{os.path.splitext(request.filename)[0]}_output.wav"
        )
